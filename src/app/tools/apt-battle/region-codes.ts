/**
 * 전국 시군구 법정동코드 (앞 5자리) 매핑
 * 출처: 행정표준코드관리시스템 (www.code.go.kr)
 */

export interface RegionMap {
  [sido: string]: { [code: string]: string };
}
...
export const REGIONS: RegionMap = {
  ...
  "경기도": {
    ...
    "41173": "안양시 동안구", "41190": "부천시",
    "41210": "광명시", "41220": "평택시",
    "41250": "동두천시", "41270": "안산시 상록구",
    "41273": "안산시 단원구",
    "41280": "고양시 덕양구", "41281": "고양시 일산동구",
    "41285": "고양시 일산서구",
    "41290": "과천시", "41310": "구리시",
...
/** 시도 내 시군구 목록 */
export function getSigunguList(sido: string): { code: string; name: string }[] {
  const region = REGIONS[sido];
  if (!region) return [];
  return Object.entries(region).map(([code, name]) => ({ code, name }));
}

/** 코드로 시군구명 찾기 */
export function getRegionName(code: string): string {
  for (const sido of Object.values(REGIONS)) {
    if (sido[code]) return sido[code];
  }
  return code;
}

/** 코드로 시도명 찾기 */
export function getSidoByCode(code: string): string {
  for (const [sido, districts] of Object.entries(REGIONS)) {
    if (districts[code]) return sido;
  }
  return "";
}

/** 전체 표시명: "서울 강남구" */
export function getFullRegionName(code: string): string {
  const sido = getSidoByCode(code);
  const sigungu = getRegionName(code);
  if (!sido) return code;
  const shortSido = sido.replace(/(특별시|광역시|특별자치시|특별자치도|도)$/, "");
